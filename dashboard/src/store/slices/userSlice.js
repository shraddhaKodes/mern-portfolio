...
const userSlice = createSlice({
  ...
  reducers: {
    ...
    logoutSuccess(state, action) {
      state.loading = false;
      state.isAuthenticated = false; // Ensure this is set to false
...
export const updateProfile = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(
      `${BASE_URL}/user/me/profile/update`,
      data,
      {
        withCredentials: true,
...
export const login = (email, password) => async (dispatch) => {
  ...
  try {
    ...
    dispatch(userSlice.actions.clearAllErrors()); // ✅ Fix: Correctly dispatching action
  } catch (error) {
    dispatch(
